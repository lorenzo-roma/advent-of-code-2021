function solution(input){
    ...
    for(const number of numbers){
        ...
        for(let i = 0; i<boards.length; i++){
            const win = evaluateWin(boards[i], number);
            if(win) toRemove.push(i);
        }
        if(toRemove.length>0) {
            lastBoard = calculatePoints(boards[toRemove.length-1], number);
        }
        while(toRemove.length) boards.splice(toRemove.pop(),1);
    }
    return lastBoard;
}
